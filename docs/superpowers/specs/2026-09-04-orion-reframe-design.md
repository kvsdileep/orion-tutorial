...
orion-tutorial/
  ...
  lessons/                       one Python file per web chapter, `# %%` cells (section 4)
    ...
    03_brain/ch13 … ch18
    README.md
  notebooks/                     generated by `orion notebooks` (jupytext); committed for Colab; header says do not edit
...
Removed: `Notebooks/` (notebooks, CONTENTS.md, README.md, test.csv, `sample_project/config.py.bak`). `Notebooks/README.md` content moves to `lessons/README.md` with the Day 10 prerequisite pointed at `langchain.agents.create_agent`.
...
| `cli.py` | `orion` command. | `check-models`, `reset` (sample_project to workspace), `sync-web`, `notebooks` | argparse |
...
Each file has a `# %% setup` cell that imports from the package, loads `.env`, and points `Workspace` at `workspace/`. Files run top to bottom with `python lessons/…/chXX.py` and cell by cell in Cursor's interactive window. Async cells use top-level `await`; when run as a script the file wraps them in `asyncio.run` through a tiny `run()` helper in the setup cell.
...
- Cell tags preserve original notebook cell numbers so the script does not need renumbering. New cells use `N<n>`.
